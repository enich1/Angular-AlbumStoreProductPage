...
describe('ProductTracklisting', () => {
  ...
  it(`should have CSS that contains a button selector @product-tracklisting-component-css4`, waitForAsync(() => {
  ...
  it(`should have CSS with a rule setting the line-height to 1 on the button selector @product-tracklisting-component-css4`, waitForAsync(() => {
    since('The ProductTracklistingComponent hasn\'t been created yet.').expect(productTracklistingCssFileExists).toBe(true);
    if(productTracklistingCssFileExists) {
      let parsed = CSSOM.parse(productTracklistingCssFile);
